...
fd_wksp_delete_named( char const * name ) {
  ...
  void * shwksp = fd_shmem_join( name, FD_SHMEM_JOIN_MODE_READ_WRITE, 0, NULL, NULL, info ); /* logs details */
...
fd_wksp_attach( char const * name ) {
  return (fd_wksp_t *)
    fd_shmem_join( name, FD_SHMEM_JOIN_MODE_READ_WRITE, 0, fd_wksp_private_join_func, NULL, NULL ); /* logs details */
}
