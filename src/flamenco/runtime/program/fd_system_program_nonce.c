...
recent_blockhashes_is_empty( fd_sysvar_cache_t const * sysvar_cache ) {
  fd_block_hash_queue_global_t const * sysvar = fd_sysvar_recent_hashes_join_const( sysvar_cache );
  if( FD_UNLIKELY( !sysvar ) ) FD_LOG_CRIT(( "Recent blockhashes sysvar is not initialized" )); /* unreachable */
  fd_block_block_hash_entry_t const * hashes = fd_recent_block_hashes_hashes_join( (void *)sysvar );
  int const is_empty = deq_fd_block_block_hash_entry_t_empty( hashes );
  deq_fd_block_block_hash_entry_t_leave( (void *)hashes );
  fd_sysvar_recent_hashes_leave_const( sysvar_cache, sysvar );
  return is_empty;
}
...
fd_check_transaction_age( fd_exec_txn_ctx_t * txn_ctx ) {
  ...
  if( fd_executor_is_blockhash_valid_for_age( block_hash_queue, recent_blockhash, FD_SYSVAR_RECENT_HASHES_CAP ) ) {
