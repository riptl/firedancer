...
fd_runtime_fuzz_txn_ctx_create( fd_runtime_fuzz_runner_t *         runner,
  ...
  fd_sysvar_cache_recover( slot_ctx );
  ...
  if( !fd_sysvar_slot_hashes_is_valid( sysvar_cache ) ) {
    ...
    fd_memset( sysvar_data+8, 0, 40 );
    fd_sysvar_cache_data_modify_commit( slot_ctx, &fd_sysvar_slot_hashes_id, 48 );
  }
  ...
  */
  ...
  fd_rent_t const rent = fd_sysvar_rent_read_nofail( sysvar_cache );
  ...
  // Save lamports per signature for most recent blockhash, if sysvar cache contains recent block hashes
  {
    ...
    fd_block_hash_queue_global_t const * rbh_sysvar = fd_sysvar_recent_hashes_join_const( sysvar_cache );
    if( rbh_sysvar ) {
      fd_block_block_hash_entry_t const * rbh_deque  = fd_recent_block_hashes_hashes_join( (void *)rbh_sysvar );
      if( rbh_deque && !deq_fd_block_block_hash_entry_t_empty( rbh_deque ) ) {
        fd_block_block_hash_entry_t const * last = deq_fd_block_block_hash_entry_t_peek_head_const( rbh_deque );
    ...
    fd_sysvar_recent_hashes_leave_const( sysvar_cache, rbh_sysvar );
  }
