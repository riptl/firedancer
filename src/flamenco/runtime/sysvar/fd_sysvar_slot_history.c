...
fd_sysvar_slot_history_update( fd_exec_slot_ctx_t * slot_ctx,
  ...
  if( FD_UNLIKELY( !fd_sysvar_cache_flags_exists( sysvar_cache->desc.slot_history.flags ) ) ) {
  ...
  fd_slot_history_global_t * history = fd_sysvar_slot_history_join( slot_ctx->sysvar_cache );
  ...
  fd_sysvar_slot_history_add( history, slot_ctx->slot );

  /* Persist write */

  fd_sysvar_slot_history_leave( slot_ctx->sysvar_cache, history );
}
