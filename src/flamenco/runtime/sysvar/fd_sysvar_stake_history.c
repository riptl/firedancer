...
fd_sysvar_stake_history_update( fd_exec_slot_ctx_t *                        slot_ctx,
  ...
  ulong const prev_slot  = fd_bank_prev_slot_get( slot_ctx->bank );
  ulong const cur_slot   = slot_ctx->slot;
  ...
  if( FD_UNLIKELY( !fd_sysvar_cache_flags_exists( sysvar_cache->stake_history.flags ) ) ) {
    fd_sysvar_stake_history_init( slot_ctx );
  }

  fd_stake_history_t * stake_history = fd_sysvar_stake_history_join( sysvar_cache );
  ...
  fd_sysvar_stake_history_leave( sysvar_cache, stake_history );
}
