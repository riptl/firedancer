...
/* This backend uses scratch pad memory as a write-back cache.  Every
   ...
   The region [seq_past,seq_wb) is outside of the write cache.

   The region [seq_wb,seq_present), i.e. spanning 'clean', 'write',
...
wq_enqueue( fd_vinyl_io_ur_t * ur,
  ...
  *sqe = (struct io_uring_sqe) {
  ...
  if( sz ) {
    ...
    if( FD_UNLIKELY( !seq ) ) FD_LOG_CRIT(( "unbalanced SQ" ));
    *sqe = (struct io_uring_sqe) {
      .opcode    = IORING_OP_WRITE,
      .fd        = 0, /* fixed file index 0 */
      .off       = dev_base,
...
wq_enqueue_seq( fd_vinyl_io_ur_t * ur,
  ...
  uchar const * spad = fd_vinyl_io_ur_spad( ur );
  ulong         sz0  = sz;
  ulong         sz1  = 0UL;

  ulong off0 = wb_ring_seq_to_off( &ur->wb, seq0 );
  ulong off1 = off0;

  if( FD_LIKELY( fd_vinyl_seq_lt( seq0, ur->wb.seq1 ) ) ) {
    sz0 = fd_ulong_min( ur->wb.seq1 - seq0, sz );
    FD_CRIT( sz0<=sz,         "invariant violation" );
    FD_CRIT( sz0<=ur->wb.sz0, "invariant violation" );
    if( sz0<sz ) {
      off1 = wb_ring_seq_to_off( &ur->wb, ur->wb.seq1 );
      FD_CRIT( sz-sz0<=ur->wb.sz1, "invariant violation" );
      sz1 = sz - sz0;
    }
  }

  FD_CRIT( off0+sz0<=ur->wb.max, "invariant violation" );
  FD_CRIT( off1+sz1<=ur->wb.max, "invariant violation" );

  if( sz0 ) wq_enqueue( ur, ur->seq_write,     spad+off0, sz0 );
  if( sz1 ) wq_enqueue( ur, ur->seq_write+sz0, spad+off1, sz1 );
  FD_CRIT( ur->seq_write+sz0+sz1==seq1, "invariant violation" );
  ur->seq_write = seq1;
...
fd_vinyl_io_ur_alloc( fd_vinyl_io_t * io,
  ...
  void * alloc = fd_vinyl_io_ur_spad( ur ) + off;
  ur->last_alloc = alloc;
  return alloc;
}
...
fd_vinyl_io_ur_append( fd_vinyl_io_t * io,
  ...
  uchar *            spad = fd_vinyl_io_ur_spad( ur );
