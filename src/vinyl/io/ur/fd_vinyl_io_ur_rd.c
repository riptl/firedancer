...
/* io_ur reads are either served by the write-back cache or by disk,
   ...
   wb_read serves the tail part of a read from the write-back cache.
   [seq0,seq0+sz) is the requested bstream range (assumed to be in
   [seq_past,seq_present)).  The tail part is written to dst.  Returns
   the number of bytes that were not read because they are not in cache.
   I.e. [seq0,seq0+retval) gives the bstream range that should be
   fetched from a file instead. */
...
wb_read( fd_vinyl_io_ur_t * io,
  ...
  /* expect most reads to hit disk */
  FD_CRIT( seq1 <= io->base->seq_present, "out-of-bounds read request" );
  ...
  ulong req1 = seq1;
  FD_CRIT( fd_vinyl_seq_ge( req0, wb_ring_seq0( &io->wb ) ), "write back cache read failed" );
  FD_CRIT( fd_vinyl_seq_le( req1, wb_ring_seq1( &io->wb ) ), "write back cache read failed" );
  ...
  uchar const * spad     = fd_vinyl_io_ur_spad( io );
  ulong const   src_off0 = wb_ring_seq_to_off( &io->wb, req0 );
  if( FD_LIKELY( fd_vinyl_seq_ge( req0, io->wb.seq1 ) ) ) {
    /* entire read is served by part1 of wb cache */
    fd_memcpy( dst, spad+src_off0, rsz );
  } else {
    /* slow path */
    ulong lsz = fd_ulong_min( io->wb.seq1 - req0, rsz );
    FD_CRIT( lsz<=rsz,        "invariant violation" );
    FD_CRIT( lsz<=io->wb.sz0, "invariant violation" );
    fd_memcpy( dst, spad+src_off0, lsz );
    if( lsz<rsz ) {
      ulong src_off1 = wb_ring_seq_to_off( &io->wb, req0+lsz );
      FD_CRIT( (rsz-lsz)<=io->wb.sz1, "invariant violation" );
      fd_memcpy( dst+lsz, spad+src_off1, rsz-lsz );
    }
  }
