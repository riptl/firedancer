...
populate_allowed_fds( fd_topo_t      const * topo,
  ...
  if( ctx->ioring_fd>=0 ) out_fds[ out_cnt++ ] = ctx->ioring_fd;
...
populate_allowed_seccomp( fd_topo_t const *      topo,
  ...
  populate_sock_filter_policy_fd_accdb_tile( out_cnt, out, (uint)fd_log_private_logfile_fd(), (uint)ctx->bstream_fd, (uint)ctx->ioring_fd );
...
vinyl_io_uring_init( fd_vinyl_tile_t * ctx,
  ...
  FD_TEST( fd_io_uring_shmem_setup( params, ctx->ioring_shmem, uring_depth, uring_depth ) );

  ctx->ioring_fd = fd_io_uring_setup( uring_depth, params );
  if( FD_UNLIKELY( ctx->ioring_fd<0 ) ) FD_LOG_ERR(( "io_uring_queue_init_params failed (%i-%s)", errno, fd_io_strerror( errno ) ));

  /* Setup io_uring file access */
  FD_TEST( 0==fd_io_uring_register_files( ctx->ioring_fd, &dev_fd, 1 ) );
  ...
  int res_err = fd_io_uring_register_restrictions( ctx->ioring_fd, res, 3U );
  if( FD_UNLIKELY( res_err<0 ) ) FD_LOG_ERR(( "io_uring_register_restrictions failed (%i-%s)", res_err, fd_io_strerror( -res_err ) ));

  /* Enable rings */
  int enable_err = fd_io_uring_enable_rings( ctx->ioring_fd );
...
privileged_init( fd_topo_t *      topo,
  ...
  ulong ctx_laddr = (ulong)ctx_laddr;
...
during_housekeeping( fd_vinyl_tile_t * ctx ) {
  ...
  if( FD_UNLIKELY( !ctx->booted ) ) {
    ...
    if( ctx->ioring_fd!=-1 ) {
      vinyl->io = fd_vinyl_io_ur_init( ctx->io_mem, IO_SPAD_MAX, ctx->bstream_fd, ctx->ioring_fd );
